...
class SetPiTimeSeries(object):
    ...
    def _pi_json(self):
        ...
        if "timeSeries" in obj:
            ...
            obj_json = obj_df.to_json(orient="records", date_format="iso").replace(
                "T00:00:00.000Z", ""
            )
    ...
    def _pi_xml(self):
        ...
        if "h" in locals():
            ...
            if "sourceOrganisation" in h:
                SubElement(sbch1, "sourceOrganisation").text = str(
                    h["sourceOrganisation"]
                )
    ...
    class _Properties(object):
        ...
        def datetime_type(self, description, key, date, time):
            ...
            self._update_properties(
                "datetime", {"key": key, "date": date, "time": time}
            )
    ...
    class _Header(object):
        ...
        def timeStep(self, unit, divider=1, multiplier=1):
            ...
            """
            self._update_header(
                "timeStep", {"unit": unit, "divider": divider, "multiplier": multiplier}
            )
        ...
        def thresholds(self, threshold_id, name, label):
            ...
            """
            self._update_header(
                "thresholds", {"id": threshold_id, "name": name, "label": label}
            )
    ...
    class _Events(object):
        ...
        def events(self, value):
            ...
            if not value.index.is_all_dates:
                raise ValueError(
                    "index does not exist or not contains datetime objects"
                )
...
class Utils(object):
    ...
    def event_client_datetime(event, tz_server, tz_client="Europe/Amsterdam"):
        ...
        event_server_time = list(
            map(int, event["time"].replace(".", ":").split(":"))
        )  # -> [HH, mm, ss, ms]
    ...
    if header in ["multiindex", "longform"]:
        ...
        for series in getTimeSeries_json.TimeSeries.series:
            ...
            if hasattr(series, "event"):
                ...
                times = [
                    (
                        *map(int, re.split("-", event["date"])),
                        *map(int, re.split(":|\.", event["time"])),
                    )
                ...
                server_times = [
                    datetime(*times[i], tzinfo=pytz.timezone(timeZone))
